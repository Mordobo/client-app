import { COUNTRIES, type Country } from "@/components/CountryPicker";
...
export default function EditProfileScreen() {
  ...
  const findCountryByName = (countryName: string | undefined): Country | null => {
    ...
    if (found) return found;
    found = COUNTRIES.find((c) => c.name.toLowerCase().includes(normalizedName.toLowerCase()) || normalizedName.toLowerCase().includes(c.name.toLowerCase()));
    return found || null;
  };
  ...
  const parsePhoneNumber = (phone: string | undefined, country: Country | null): { extension: string; number: string } => {
    ...
    const allExtensions = Array.from(new Set(COUNTRIES.map((c) => c.phoneExtension))).sort((a, b) => b.length - a.length);
  ...
  React.useEffect(() => {
    if (user) {
      ...
      setFormData((prev) => {
        const shouldUpdate = prev.firstName !== (user.firstName || "") || prev.lastName !== (user.lastName || "") || prev.email !== (user.email || "") || prev.phoneExtension !== phoneExtension || prev.phoneNumber !== phoneNumber || prev.dateOfBirth?.getTime() !== dateOfBirth?.getTime() || prev.gender !== user.gender || prev.country?.name !== foundCountry?.name;
    ...
    try {
      ...
      if (formData.phoneExtension && formData.phoneNumber) {
        if (formData.phoneNumber.trim().length < 6) {
          Alert.alert(t("common.error"), t("errors.phoneNumberTooShort") || "Phone number must be at least 6 characters");
          setLoading(false);
          return;
        }
      ...
      const updatedAvatar = (apiUser.profile_image as string | undefined) || (imageUri && imageUri !== user.avatar ? imageUri : user.avatar);
      ...
      const errorMessage = error instanceof Error ? error.message : t("errors.updateProfileFailed");
